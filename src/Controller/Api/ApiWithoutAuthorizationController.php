...
class ApiWithoutAuthorizationController extends AbstractController
{
        ...
        try {
            ...
            if (is_array($result)) {
                foreach ($result as $value) {
                    ...
                    $value['imagen'] = !empty($value['imagen']) ? $this->baseUrl . '/uploads/images/conocenos/imagen/' . $value['imagen'] : null;
                    $response[] = $value;
                }
            }
            return $this->json(['messaje' => 'OK', 'data' => $response]);
    ...
    public function listarPoliticaReservacion(PoliticaReservacionRepository $politicaReservacionRepository)
    {
        try {
            ...
            $response = [];
            foreach ($result as $value) {
                $value['imagen'] = !empty($value['imagen']) ? $this->baseUrl . "/uploads/images/politica_reservacion/imagen/" . $value['imagen'] : null;
                $response[] = $value;
            }
            return $this->json(['messaje' => 'OK', 'data' => $response[0]]);
        } catch (Exception $exc) {
            return $this->json(['messaje' => $exc->getMessage(), 'data' => []], Response::HTTP_BAD_GATEWAY);
        }
    }
